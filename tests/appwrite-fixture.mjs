...
export async function startFixture({glb=Buffer.from('fixture bytes'),ring=glb}={}){
 ...
 const server=createServer(async(req,res)=>{try{
  ...
  if(p[1]==='storage'){
   ...
   if(files.has(p[5])){const {info,bytes}=files.get(p[5]);if(p.at(-1)==='view'){const match=/bytes=(\d+)-(\d*)/.exec(req.headers.range||'');const start=match?Number(match[1]):0,end=match&&match[2]?Number(match[2]):bytes.length-1;res.writeHead(match?206:200,{'content-type':'video/mp4','accept-ranges':'bytes',...(match?{'content-range':`bytes ${start}-${end}/${bytes.length}`}:{})});return res.end(bytes.subarray(start,end+1));}return send(info);}
