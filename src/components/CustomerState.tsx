...
const Context=createContext<{user:CustomerUser|null;guest:boolean;state:CustomerState;catalog:AssetManifest;ready:boolean;error:string|null;reloadState:()=>Promise<void>}>({user:null,guest:false,state:empty,catalog:canonicalAssetManifest,ready:false,error:null,reloadState:async()=>{}});
export function CustomerProvider({children}:{children:ReactNode}){
  const [user,setUser]=useState<CustomerUser|null>(null),[guest,setGuest]=useState(false),[state,setState]=useState(empty),[catalog,setCatalog]=useState<AssetManifest>(canonicalAssetManifest),[ready,setReady]=useState(false),[error,setError]=useState<string|null>(null);
