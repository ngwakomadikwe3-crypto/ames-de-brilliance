...
function Shell({children}:{children:React.ReactNode}){return <main className="ames-jeweller-portal min-h-screen bg-[#0d1115] px-5 py-8 text-[#f0ede7]"><div className="mx-auto max-w-4xl"><Link href="/" className="text-xs tracking-[.24em] text-[#dce2ea]">AMES</Link><div className="mt-10">{children}</div></div></main>}
export default async function JewellerPortal(){
 ...
 if(response.status===401)return <Shell><h1 className="text-3xl font-light">Jeweller portal</h1><p className="mt-4 text-[#aeb5bc]">Sign in with the AMES account using the email on your application, then return here.</p><Link className="mt-7 inline-block border border-white/25 bg-white/[.04] px-5 py-3 text-sm text-white" href="/account">AMES account sign in</Link></Shell>;
