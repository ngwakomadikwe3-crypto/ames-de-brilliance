...
export default function VideoSection(){
 ...
 const reload=async()=>{const data=await request('/api/videos');if(!Array.isArray(data))throw Error('Video records unavailable. Please retry.');setRows(data);};
 useEffect(()=>{void reload().catch(e=>setMessage(e.message));void request('/api/customer/catalog').then(d=>{if(!Array.isArray(d.assets))throw Error('Product links unavailable.');setProducts(d.assets.filter((a:Row)=>a.category!=='stone'&&['PUBLIC','public'].includes(a.accessTier)));}).catch(()=>setMessage('Approved product links are unavailable. Video publishing can be used without a product link.'));},[]);
 ...
 <div>{!rows.length&&<p>No videos loaded.</p>}<button disabled={busy} onClick={()=>{reload().then(()=>setMessage('')).catch(e=>setMessage(e.message));}}>Reload videos</button>{rows.map(v=><button key={v.id} style={{display:'block',padding:10}} onClick={()=>setForm(v)}>{v.title||v.caption||'Untitled'} / {v.status}</button>)}</div>
