...
export default async function Home() {
  ...
  return (
    <div className="w-full">
      ...
      `}</style>
      <div className="relative w-full flex flex-col items-center justify-center py-16 md:py-24" style={{ background: '#EAE8E4', minHeight: 420 }}>
        ...
        <div className="relative w-full max-w-[420px] mb-8" style={{ background: '#1A1A1A', borderRadius: 16, border: '1px solid #A6A6AB', overflow: 'hidden' }}>
          <div style={{ height: 280, position: 'relative', overflow: 'hidden' }}>
            ...
            <svg viewBox="0 0 24 24" fill="none" style={{ position: 'absolute', top: '50%', left: '50%', transform: 'translate(-50%, -50%)', width: 140, height: 140 }} aria-hidden="true">
              <path d="M12 2L22 9L12 22L2 9L12 2Z" stroke="#A6A6AB" strokeWidth="1.5" strokeLinejoin="round" fill="none" />
            </svg>
            {/* Sparkle 1 — top-left */}
            <div className="hero-sparkle-1" style={{ position: 'absolute', top: 36, left: 42, pointerEvents: 'none' }}>
              <svg width="16" height="16" viewBox="0 0 14 14" fill="none"><path d="M7 0L8.2 5.2L14 7L8.2 8.8L7 14L5.8 8.8L0 7L5.8 5.2Z" fill="#A6A6AB" fillOpacity="0.9"/></svg>
            </div>
            {/* Sparkle 2 — top-right */}
            <div className="hero-sparkle-2" style={{ position: 'absolute', top: 52, right: 46, pointerEvents: 'none' }}>
              <svg width="12" height="12" viewBox="0 0 14 14" fill="none"><path d="M7 0L8.2 5.2L14 7L8.2 8.8L7 14L5.8 8.8L0 7L5.8 5.2Z" fill="#A6A6AB" fillOpacity="0.7"/></svg>
            </div>
          </div>
        </div>
        {/* Wordmark below card */}
        <div className="flex flex-col items-center mb-6">
          <svg viewBox="0 0 24 24" fill="none" style={{ width: 44, height: 44 }} aria-hidden="true">
            <path d="M12 2L22 9L12 22L2 9L12 2Z" stroke="#A6A6AB" strokeWidth="1.8" strokeLinejoin="round" fill="none" />
          </svg>
          <span className="mt-3 text-[28px] md:text-[36px] font-semibold tracking-[-0.01em]" style={{ fontFamily: 'Georgia, "Times New Roman", serif', color: '#171717' }}>AMES</span>
          <span className="mt-1 text-[10px] md:text-[11px] font-normal uppercase tracking-[0.3em]" style={{ color: '#A6A6AB' }}>DE BRILLIANTE</span>
        </div>
        <p className="text-[13px] font-light leading-relaxed max-w-md text-center" style={{ color: '#6E6C69' }}>
      ...
      <p className="text-[11px] mb-8 leading-relaxed" style={{ color: "#6E6C69" }}>
      ...
      {stones.length === 0 ? (
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-4">
          {stones.map((stone) => {
            ...
            return (
              <div key={stone.id} className="overflow-hidden stone-glow" style={{ background: '#FCFCFB', borderRadius: 12, border: '1px solid rgba(23,23,23,0.08)' }}>
                <div className="aspect-square overflow-hidden relative">
                  ...
                  />
                  {isRough && (
                    <span className="absolute top-2 left-2 bg-[#A6A6AB] text-[#EAE8E4] text-[9px] font-bold uppercase tracking-wider px-1.5 py-0.5">
                      Rough
                    </span>
                  )}
                  {stone.trader_preferred && (
                    <span className="absolute top-2 right-2 flex items-center gap-1 text-[9px] font-medium uppercase tracking-[0.08em] px-2 py-0.5 rounded-full" style={{ color: "#A6A6AB", background: "rgba(22,24,26,0.9)", border: "1px solid #A6A6AB" }}>
                ...
                <div className="p-3">
                  <div className="flex items-center justify-between mb-1.5">
                    ...
                    <span className="text-[11px] font-medium tabular-nums" style={{ color: stone.price ? "#A6A6AB" : "#6E6C69" }}>
                  ...
                  {isRough ? (
                    <div className="text-[11px] text-muted leading-relaxed">
                      <div className="font-medium text-[#171717] mb-0.5">{stone.category} · {stone.crystal_form}</div>
                      <div>{stone.carat}ct · {stone.color}</div>
                      {stone.clarity_notes && <div className="mt-0.5 italic">{stone.clarity_notes}</div>}
                      {stone.kp_status === 1 && (
                        <div className="mt-1 text-[10px] font-semibold text-[#171717]">KP cert on file</div>
                      )}
                    </div>
                  ) : (
                    <div className="text-[11px] text-muted leading-relaxed">
                  ...
                  <a
                    ...
                    className="block w-full mt-2 py-1.5 bg-[#A6A6AB] text-[#EAE8E4] text-[11px] font-medium text-center rounded"
                  >
                    Enquire on WhatsApp
                  </a>
                </div>
      ...
      <p className="text-center text-[12px] text-muted mt-8">
        ...
        <a href="/request" className="underline text-[#171717]">Submit a sourcing request</a>.
      </p>
    </div>
    </div>
  );
