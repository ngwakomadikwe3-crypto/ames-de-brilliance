...
const EMPTY_DB = new Proxy({} as DbService, {
  get: () => async () => ({ documents: [], total: 0 }),
});
function getSafeDb(): DbService {
  return getDbSvc() || EMPTY_DB;
}
type StorageService = NonNullable<ReturnType<typeof getStorage>>;
const EMPTY_STORAGE = new Proxy({} as StorageService, {
  get: () => async () => ({ $id: "" }),
});
function getSafeStorage(): StorageService {
  return getStorage() || EMPTY_STORAGE;
}
...
export async function getRequestById(id: string): Promise<DbRequest | undefined> {
  await ensureReady();
  try { return doc<DbRequest>(await getSafeDb().getDocument({ databaseId: DB_ID, collectionId: "requests", documentId: id })); } catch { return undefined; }
}
...
export async function addRequest(data: { buyer_name: string; company: string; country: string; contact: string; type: string; shape: string; carat_min: string; carat_max: string; color: string; clarity: string; certification: string; notes: string; kp_licence?: string; kp_country?: string; consent?: boolean; declaration?: boolean; consent_timestamp?: string; }): Promise<DbRequest> {
  await ensureReady();
  const db = getSafeDb(); const now = nowISO();
...
export async function updateRequest(id: string, updates: Partial<Pick<DbRequest, "status" | "notes" | "mandate">>): Promise<DbRequest | null> {
  ...
  await getSafeDb().updateDocument({ databaseId: DB_ID, collectionId: "requests", documentId: id, data: updates });
...
export async function updateRequestOffer(id: string, offerText: string): Promise<DbRequest | null> {
  await ensureReady();
  if (!(await getRequestById(id))) return null;
  await getSafeDb().updateDocument({ databaseId: DB_ID, collectionId: "requests", documentId: id, data: { offer_text: offerText, offer_timestamp: nowISO(), status: "Quoted" } });
  return (await getRequestById(id)) || null;
}
...
export async function createTrader(name: string, whatsapp: string): Promise<DbTrader> {
  await ensureReady();
  const res = await getSafeDb().createDocument({ databaseId: DB_ID, collectionId: "traders", documentId: ID.unique(), data: { name, whatsapp, licence: "", portal_code: genPortalCode(), email: "", status: "Active", company: "", country: "", licence_photo: "", created_at: nowISO() } });
  return doc<DbTrader>(res);
}

export async function getAllTraders(): Promise<DbTrader[]> {
  await ensureReady();
  const res = await getSafeDb().listDocuments({ databaseId: DB_ID, collectionId: "traders" });
...
export async function getTraderByPortalCode(code: string): Promise<DbTrader | undefined> {
  await ensureReady();
  try { const res = await getSafeDb().listDocuments({ databaseId: DB_ID, collectionId: "traders", queries: [Query.equal("portal_code", code), Query.limit(1)] }); return res.documents.length ? doc<DbTrader>(res.documents[0]) : undefined; } catch { return undefined; }
}
...
export async function getTraderById(id: string): Promise<DbTrader | undefined> {
  await ensureReady();
  try { return doc<DbTrader>(await getSafeDb().getDocument({ databaseId: DB_ID, collectionId: "traders", documentId: id })); } catch { return undefined; }
}
...
export async function addTraderApplication(data: { name: string; company: string; country: string; whatsapp: string; email: string; licence: string; licence_photo: string }): Promise<DbTrader> {
  await ensureReady();
  const res = await getSafeDb().createDocument({ databaseId: DB_ID, collectionId: "traders", documentId: ID.unique(), data: { name: data.name, company: data.company, country: data.country, whatsapp: data.whatsapp, email: data.email, licence: data.licence, licence_photo: data.licence_photo, portal_code: "", status: "Pending", created_at: nowISO() } });
  return doc<DbTrader>(res);
}
...
export async function approveTrader(id: string): Promise<DbTrader | null> {
  ...
  await getSafeDb().updateDocument({ databaseId: DB_ID, collectionId: "traders", documentId: id, data: { status: "Active", portal_code: existing.portal_code || genPortalCode() } });
...
export async function declineTrader(id: string): Promise<DbTrader | null> {
  ...
  await getSafeDb().updateDocument({ databaseId: DB_ID, collectionId: "traders", documentId: id, data: { status: "Declined" } });
  return (await getTraderById(id)) || null;
}
...
export async function togglePreferredTrader(id: string): Promise<boolean> {
  ...
  await getSafeDb().updateDocument({ databaseId: DB_ID, collectionId: "traders", documentId: id, data: { preferred: newVal } });
  return newVal;
}
...
export async function getTraderReports(traderId: string): Promise<DbReport[]> {
  await ensureReady();
  const res = await getSafeDb().listDocuments({ databaseId: DB_ID, collectionId: "reports", queries: [Query.equal("trader_id", traderId), Query.orderDesc("report_date")] });
  return res.documents.map(d => doc<DbReport>(d));
}

export async function getAllReports(): Promise<(DbReport & { trader_name: string })[]> {
  await ensureReady();
  const res = await getSafeDb().listDocuments({ databaseId: DB_ID, collectionId: "reports", queries: [Query.orderDesc("report_date")] });
...
export async function addReport(traderId: string, periodStart: string, periodEnd: string, summary: string, data: object): Promise<DbReport> {
  await ensureReady();
  const now = nowISO();
  const res = await getSafeDb().createDocument({ databaseId: DB_ID, collectionId: "reports", documentId: ID.unique(), data: { trader_id: traderId, period_start: periodStart, period_end: periodEnd, report_date: now, summary, data: JSON.stringify(data), created_at: now } });
...
export async function generateWeeklyReport(traderId: string): Promise<DbReport | null> {
  ...
  const db = getSafeDb(); const now = new Date(); const weekAgo = new Date(now.getTime() - 7 * 86400000);
...
export async function getStoneById(id: string) {
  await ensureReady();
  try { const enriched = await enrichStones([doc<any>(await getSafeDb().getDocument({ databaseId: DB_ID, collectionId: "stones", documentId: id }))]); return enriched[0] || null; } catch { return null; }
}
...
export async function addStone(data: { stone_type: string; shape: string; carat: number; color: string; clarity: string; cut: string; certification: string; category: string; crystal_form: string; clarity_notes: string; kp_status: boolean; price: number | null; status: string; photo: string; source: string; trader_id: string | null; commission: number; photo_path: string | null; listing_category?: string; }) {
  await ensureReady(); const db = getSafeDb(); const now = nowISO();
...
export async function updateStone(id: string, updates: Partial<Pick<DbStone, "status" | "sale_price" | "price" | "photo" | "photo_path" | "shape" | "carat" | "color" | "clarity" | "cut" | "certification" | "listing_category">>) {
  ...
  await getSafeDb().updateDocument({ databaseId: DB_ID, collectionId: "stones", documentId: id, data });
  if (updates.status !== undefined && updates.status !== existing.status) {
    await getSafeDb().createDocument({ databaseId: DB_ID, collectionId: "stone_status_log", documentId: ID.unique(), data: { stone_id: id, status: updates.status, reason: "", changed_at: nowISO() } });
  }
  if (updates.status === "Sold" && existing.status !== "Sold") {
    const salePrice = updates.sale_price ?? existing.sale_price ?? 0;
    const videoRes = await getSafeDb().listDocuments({ databaseId: DB_ID, collectionId: "videos", queries: [Query.equal("stone_id", id)] });
    for (const v of videoRes.documents) { if (v.model_id) { await getSafeDb().updateDocument({ databaseId: DB_ID, collectionId: "videos", documentId: v.$id, data: { sales_count: (v.sales_count || 0) + 1, sales_value: (v.sales_value || 0) + (salePrice || 0), commission_earned: (v.commission_earned || 0) + ((salePrice || 0) * 0.005) } }); } }
  }
  return getStoneById(id);
}
...
export async function approveStone(id: string, edits: Partial<Pick<DbStone, "shape" | "carat" | "color" | "clarity" | "cut" | "certification" | "price" | "listing_category">>) {
  ...
  await getSafeDb().updateDocument({ databaseId: DB_ID, collectionId: "stones", documentId: id, data });
  await getSafeDb().createDocument({ databaseId: DB_ID, collectionId: "stone_status_log", documentId: ID.unique(), data: { stone_id: id, status: "Available", reason: "Approved", changed_at: nowISO() } });
  return getStoneById(id);
}

export async function rejectStone(id: string, reason: string) {
  await ensureReady();
  await getSafeDb().updateDocument({ databaseId: DB_ID, collectionId: "stones", documentId: id, data: { status: "Rejected" } });
  await getSafeDb().createDocument({ databaseId: DB_ID, collectionId: "stone_status_log", documentId: ID.unique(), data: { stone_id: id, status: "Rejected", reason, changed_at: nowISO() } });
  return getStoneById(id);
}

export async function getStoneStatusLog(stoneId: string): Promise<DbStoneStatusLog[]> {
  await ensureReady(); const res = await getSafeDb().listDocuments({ databaseId: DB_ID, collectionId: "stone_status_log", queries: [Query.equal("stone_id", stoneId), Query.orderAsc("changed_at")] });
...
export async function getAllOrders() {
  await ensureReady(); const db = getSafeDb();
...
export async function updateOrderStatus(id: string, status: string): Promise<DbOrder | null> {
  await ensureReady();
  try { await getSafeDb().updateDocument({ databaseId: DB_ID, collectionId: "orders", documentId: id, data: { status } }); return doc<DbOrder>(await getSafeDb().getDocument({ databaseId: DB_ID, collectionId: "orders", documentId: id })); } catch { return null; }
}
...
export async function addVideo(videoUrl: string, caption: string, stoneId: string | null): Promise<DbVideo> {
  ...
  const res = await getSafeDb().createDocument({ databaseId: DB_ID, collectionId: "videos", documentId: ID.unique(), data: { video_url: videoUrl, caption, stone_id: stoneId || "", published: false, model_id: "", status: "Live", tap_count: 0, reserve_count: 0, sales_count: 0, sales_value: 0, commission_earned: 0, likes_count: 0, house_note: "", featured_piece: "", created_at: now } });
...
export async function updateVideo(id: string, updates: { video_url?: string; caption?: string; stone_id?: string | null; published?: number | boolean }): Promise<DbVideo | null> {
  await ensureReady(); const db = getSafeDb(); const data: any = {};
...
export async function deleteVideo(id: string): Promise<boolean> { await ensureReady(); try { await getSafeDb().deleteDocument({ databaseId: DB_ID, collectionId: "videos", documentId: id }); return true; } catch { return false; } }
...
export async function getActiveModelCount(): Promise<number> {
  await ensureReady();
  const res = await getSafeDb().listDocuments({ databaseId: DB_ID, collectionId: "models", queries: [Query.equal("status", "Active"), Query.limit(0)] });
  return res.total;
}

export async function getModelByPortalCode(code: string): Promise<DbModel | undefined> { await ensureReady(); try { const res = await getSafeDb().listDocuments({ databaseId: DB_ID, collectionId: "models", queries: [Query.equal("portal_code", code), Query.limit(1)] }); return res.documents.length ? doc<DbModel>(res.documents[0]) : undefined; } catch { return undefined; } }
export async function getModelById(id: string): Promise<DbModel | undefined> { await ensureReady(); try { return doc<DbModel>(await getSafeDb().getDocument({ databaseId: DB_ID, collectionId: "models", documentId: id })); } catch { return undefined; } }

export async function addModel(name: string, whatsapp: string, instagram: string): Promise<DbModel> {
  await ensureReady();
  const res = await getSafeDb().createDocument({ databaseId: DB_ID, collectionId: "models", documentId: ID.unique(), data: { name, whatsapp, instagram, portal_code: genPortalCode(), status: "Active", monthly_video_quota: 30, monthly_base_fee: 200, commission_rate: 0.005, payment_method: "", payment_details: "", total_paid: 0, created_at: nowISO() } });
...
export async function getModelMonthlySummary(modelId: string) {
  ...
  const monthStart = new Date(new Date().getFullYear(), new Date().getMonth(), 1).toISOString();
  const res = await getSafeDb().listDocuments({ databaseId: DB_ID, collectionId: "videos", queries: [Query.equal("model_id", modelId), Query.equal("status", "Live")] });
...
export async function getModelPaymentReport(modelId: string) {
  await ensureReady(); const model = (await getModelById(modelId))!; const db = getSafeDb();
...
export async function markModelPaid(modelId: string, amount: number): Promise<void> {
  ...
  await getSafeDb().updateDocument({ databaseId: DB_ID, collectionId: "models", documentId: modelId, data: { total_paid: model.total_paid + amount } });
}

export async function approveModelVideo(id: string, extra?: { house_note?: string; featured_piece?: string }): Promise<DbVideo | null> {
  await ensureReady(); try { await getSafeDb().updateDocument({ databaseId: DB_ID, collectionId: "videos", documentId: id, data: { status: "Live", published: true, ...extra } }); return doc<DbVideo>(await getSafeDb().getDocument({ databaseId: DB_ID, collectionId: "videos", documentId: id })); } catch { return null; }
}

export async function declineModelVideo(id: string): Promise<boolean> { await ensureReady(); try { await getSafeDb().deleteDocument({ databaseId: DB_ID, collectionId: "videos", documentId: id }); return true; } catch { return false; } }

export async function incrementTapCount(id: string): Promise<void> {
  await ensureReady(); try { const v = await getSafeDb().getDocument({ databaseId: DB_ID, collectionId: "videos", documentId: id }); await getSafeDb().updateDocument({ databaseId: DB_ID, collectionId: "videos", documentId: id, data: { tap_count: (v.tap_count || 0) + 1 } }); } catch {}
}
...
export async function saveLicenceDoc(filename: string, buffer: Buffer): Promise<string> {
  ...
  const file = InputFile.fromBuffer(buffer, `${Date.now()}_${safeName}`);
  const res = await getSafeStorage().createFile({ bucketId: LICENCE_DOCS_BUCKET, fileId: ID.unique(), file });
...
export async function getStoneSalesCount(stoneId: string): Promise<number> {
  await ensureReady();
  const res = await getSafeDb().listDocuments({ databaseId: DB_ID, collectionId: "orders", queries: [Query.equal("stone_id", stoneId), Query.equal("status", "Paid")] });
  return res.total;
}
...
export async function getStaffByCode(code: string): Promise<DbStaff | undefined> {
  await ensureReady();
  try {
    const res = await getSafeDb().listDocuments({ databaseId: DB_ID, collectionId: "staff", queries: [Query.equal("access_code", code), Query.limit(1)] });
...
export async function enableTrader(id: string): Promise<DbTrader | null> {
  await ensureReady();
  if (!(await getTraderById(id))) return null;
  await getSafeDb().updateDocument({ databaseId: DB_ID, collectionId: "traders", documentId: id, data: { status: "Active" } });
  return (await getTraderById(id)) || null;
}

export async function getTraderStoneCount(traderId: string): Promise<number> {
  await ensureReady();
  const res = await getSafeDb().listDocuments({ databaseId: DB_ID, collectionId: "stones", queries: [Query.equal("trader_id", traderId), Query.limit(0)] });
  return res.total;
}

export async function getTraderSalesCount(traderId: string): Promise<number> {
  await ensureReady();
  const stones = await getSafeDb().listDocuments({ databaseId: DB_ID, collectionId: "stones", queries: [Query.equal("trader_id", traderId)] });
  let count = 0;
...
export async function disableModel(id: string): Promise<boolean> {
  await ensureReady();
  try {
    await getSafeDb().updateDocument({ databaseId: DB_ID, collectionId: "models", documentId: String(id), data: { status: "Disabled" } });
    return true;
  } catch { return false; }
}
...
export async function enableModel(id: string): Promise<boolean> {
  await ensureReady();
  try {
    await getSafeDb().updateDocument({ databaseId: DB_ID, collectionId: "models", documentId: String(id), data: { status: "Active" } });
    return true;
  } catch { return false; }
}
...
export async function getModelVideoStats(modelId: string): Promise<{ count: number; views: number; likes: number }> {
  await ensureReady();
  const res = await getSafeDb().listDocuments({ databaseId: DB_ID, collectionId: "videos", queries: [Query.equal("model_id", String(modelId))] });
...
export async function logUsage(service: string, model: string, promptTokens: number, completionTokens: number, route: string): Promise<void> {
  ...
  try {
    await getSafeDb().createDocument({ databaseId: DB_ID, collectionId: "usage_log", documentId: ID.unique(), data: {
...
export async function getUsageStats(startDate: string): Promise<{ totalCalls: number; totalTokens: number; totalCost: number; byDay: Record<string, { calls: number; tokens: number; cost: number }> }> {
  await ensureReady();
  const res = await getSafeDb().listDocuments({ databaseId: DB_ID, collectionId: "usage_log", queries: [Query.greaterThanEqual("created_at", startDate)] });
...
export async function getBalances(): Promise<any[]> {
  await ensureReady();
  const res = await getSafeDb().listDocuments({ databaseId: DB_ID, collectionId: "balances" });
...
export async function updateBalance(service: string, amount: number, note: string): Promise<void> {
  await ensureReady();
  const res = await getSafeDb().listDocuments({ databaseId: DB_ID, collectionId: "balances", queries: [Query.equal("service", service), Query.limit(1)] });
  if (res.documents.length) {
    await getSafeDb().updateDocument({ databaseId: DB_ID, collectionId: "balances", documentId: res.documents[0].$id, data: { amount, note, updated_at: nowISO() } });
  } else {
    await getSafeDb().createDocument({ databaseId: DB_ID, collectionId: "balances", documentId: ID.unique(), data: { service, amount, note, updated_at: nowISO() } });
  }
}
